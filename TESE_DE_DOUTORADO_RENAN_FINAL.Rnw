...
setwd("C:/Users/Windows 8.1/Desktop/Tese Doutorado")
@
...
pib <- read.csv2("pib_introducao.csv", header = T, dec = ",") %>%
       mutate(X = 1:56)
...
rs_df <- read.csv("DFEstEnsaio2.csv", header=T, sep=";", dec=",")
...
\begin{routput}[H]
<<echo=false>>=
base_bolha <- readRDS("base_bolha.rds")
...
base_frota <- readRDS("base_frota.rds") %>% 
